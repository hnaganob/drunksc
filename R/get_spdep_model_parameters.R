get_spdep_model_parameters <- function(time = NULL, sp_data, listw) {
  ...
  intercept <- intercept_se <- intercept_p_value <- lambda <- lambda_se <- lambda_p_value <- sigma2 <- family <- style <- numeric(length(time))
  ...
  for (frame in seq_along(time)) {
    ...
    family[frame] <- model$family
    style[frame] <- listw$style
